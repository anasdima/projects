...
__global__ void compute_distance(float* query, float* data, float* dist, int* idx, int numObjects, int numAttributes){
	...
	if(tid<numObjects) {
		...
		dist[element] = euclidean_distance(&data[element*numAttributes], &Qs[0], numAttributes);	
		idx[element] = tid;
	}
}
...
__global__ void reduce(float* dist, int* idx, int N, int stride){
	...
	if(tid < (stride-1) ) {
		...
		if(dist[element]>dist[element+stride]) { // Move minimum distances to the first half
	...
	for(i=0;i<numQueries;i++) {
		...
		for(j=0;j<k;j++) {
			...
			while(stride > 1) {
				...
				//printf("threads = %d | stride = %d\n", (int)tmp_grid_size*BlockSize, stride);
...
int main(int argc, char **argv){
	...
	/*======== Copy training_set and queries from files =========*/
	FILE *dataset, *queries;
	
	dataset = fopen("data.bin", "rb");
	if(dataset == NULL){printf("Error opening data.bin\n");}
	size_t a = fread(training_set.data, sizeof(float), numObjects*numDim, dataset);
	if(a!=numObjects*numDim){printf("Error reading data from data.bin\n");}
	fclose(dataset);
	
	queries = fopen("query.bin", "rb");
	if(queries == NULL){printf("Error opening query.bin\n");}
	size_t b = fread(query_set.data, sizeof(int), numQueries*numDim, queries);
	if(b!=numQueries*numDim){printf("Error reading data from query.bin\n");}
	fclose(queries);
