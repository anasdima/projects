...
int main(int argc,char **argv) {
  ...
  if (world_rank == 0) {
    ...
    size_t w = fread(temp_dataset, sizeof(double), N*d, fp);
  ...
  if (numObjects % world_size == 0) { // If numObjects is a multiple of number of processes then subworkloads are equal in size
    ...
    MPI_Scatter(temp_dataset, subworkload*data_in.leading_dim, MPI_DOUBLE, data_in.dataset, subworkload*data_in.leading_dim, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    MPI_Scatterv(temp_dataset, sizes, displs, MPI_DOUBLE, data_in.dataset, subworkload*data_in.leading_dim, MPI_DOUBLE, 0, MPI_COMM_WORLD);
